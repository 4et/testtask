...
class RegistrationModel extends Model
{
    ...
    function signUp()
    {
        ...
        $select = "SELECT * FROM `somedata` WHERE `email` in('$email')";
        $res = $this->db->query($select);
        $row = $res->fetch();
        if ($row == false) {
            $select = "SELECT `ter_name` FROM `cities` WHERE `ter_id` = ?";
            $res = $this->db->prepare($select);
            $ci = $this->mas['city'];
            $res->execute(array($ci));
            $row = $res->fetch();
            $city = $row['ter_name'];
            $sel2 = "SELECT `ter_name` FROM `cities` WHERE `ter_id` = ?";
            $res = $this->db->prepare($sel2);
            $ar = $this->mas['area'];
            $res->execute(array($ar));
            $row = $res->fetch();
            $area = $row['ter_name'];
            $sel3 = "SELECT `ter_name` FROM `cities` WHERE `ter_id` = ?";
            ...
            $insert = "INSERT INTO `somedata`(`name`, `email`, `territory`) VALUES (?, ?, ?)";
